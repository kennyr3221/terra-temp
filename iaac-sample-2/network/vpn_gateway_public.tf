resource "azurerm_public_ip" "project1_vpn_ip" {
  name                            = "project1_vpn_ip"
  location                        = azurerm_resource_group.project1_rg.location
  resource_group_name             = azurerm_resource_group.project1_rg.project1_vpn_ip
  allocation_method               = "Dynamic"
}

resource "azurerm_virtual_network_gateway" "project1_vpn_gateway" {
  name                           = "project1_vpn_gateway"
  location                       = azurerm_resource_group.project1_rg.location
  resource_group_name            = azurerm_resource_group.project1_rg.project1_vpn_gateway
  type                           = "Vpn"
  vpn_type                       = "RouteBased"
  sku                            = "VpnGw1"
  active_active                  = false
